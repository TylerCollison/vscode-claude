...
post_to_mattermost() {
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        if [ -n "$post_id" ] && [ "$post_id" != "null" ] && [ "$post_id" != "" ]; then
            ...
            local thread_id_file="/tmp/mm_thread_id"
            if echo "$post_id" > "$thread_id_file"; then
                log_success "Thread ID written to $thread_id_file"
            else
                log_warning "Failed to write thread ID to $thread_id_file"
            fi
        else
            error_exit "Message post failed - check Mattermost server configuration"
        fi
        ...
        # Extract post_id using grep/cut for fallback
        local post_id
        ...
        if [ -n "$post_id" ]; then
            ...
            log_warning "Could not extract post ID from response"
        fi
    fi
