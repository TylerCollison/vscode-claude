...
## Overview
ClaudeConX allows you to configure which docker image is used for instances, supporting custom docker images while maintaining backward compatibility with existing deployments.
...
```bash
# Use custom image
cconx start my-instance --image custom-registry/vscode-claude:dev

# Use global default
cconx start my-instance

# Use specific tag of default image (backward compatible)
cconx start my-instance --image tylercollison2089/vscode-claude:stable
```
...
## Configuration
Configure the default image in the global configuration file (`~/.cconx/global-config.json`):
