...
**Goal:** On every git commit in the parent workspace repo, create a dispatched worker (swarm service or local container) for each currently-ready Beads task. The dispatcher sets the branch name for the worker, and the worker auto-creates it off the default branch.

**Architecture:** A Python 3 stdlib daemon (opt-in via `BEADS_DISPATCH=true`). A root daemon installs a `post-commit` hook into the workspace repo and listens on a unix socket. The hook (runs as `abc`) pings the socket; the daemon reads the ready set (`bd list --ready --json`), and for each unseen ready task: dispatch via `docker service create` (swarm manager) or `docker run -d`.
...
- [ ] **Step 13: [DELETED] Implement `git_current_branch(workspace)`**
- [ ] **Step 14: [DELETED] Implement `branch_exists_remote(workspace, branch, repo_url)`**
- [ ] **Step 15: [DELETED] Implement `push_task_branch(workspace, branch, repo_url)`**
...
- [x] **Step 18: Implement `dispatch_all(cfg, self_info, seen)`** — for each ready issue not in `seen`: re-check ready, `find_free_host_port`, detect swarm, dispatch; mark seen + persist.
